...
Tunnel.prototype.addParticle = function() {
  this.textGeometry = new THREE.PlaneBufferGeometry(0.5, 0.5)
  this.geometry = new THREE.RingBufferGeometry(1, 3, 1)
...
Tunnel.prototype.render = function(time) {
  ...
  if(!global.hamburgerIsOpen) {
    ...
    this.intersectsMesh = raycaster.intersectObjects(global.arrForMesh)
...
function Particle (scene, burst, canvas, i, texture, color, text, newPosition, geometry, textGeometry) {
  ...
  if (this.burst){
    this.speed = 0;
    this.textMesh.scale.x *= 5.46;
    this.textMesh.scale.y *= 5.46;
...
Particle.prototype.update = function (tunnel) {
  ...
  this.textMesh.position.x = this.pos.x + this.positionForText[this.i][0] + 0.001 + 0.0019
  this.textMesh.position.y = this.pos.y + 0.0098 + this.positionForText[this.i][1] // +
  this.textMesh.position.z = this.pos.z - 0.001;
